...
if [[ -d "${BOOST_BASENAME}-install" ]] ; then
  ...
  wget --no-check-certificate "https://dl.bintray.com/boostorg/release/${BOOST_VERSION}/source/boost_${BOOST_VERSION//./_}.tar.gz"
  ...
  ./b2 toolset="${BOOST_TOOLSET}" cxxflags="${BOOST_CFLAGS}" --prefix="../${BOOST_BASENAME}-install" --with-python include="/home/buaacvis/anaconda3/envs/carla0.9.4/include/python3.5m/" -j ${CARLA_BUILD_CONCURRENCY} stage release 
  ./b2 toolset="${BOOST_TOOLSET}" cxxflags="${BOOST_CFLAGS}" --prefix="../${BOOST_BASENAME}-install" --with-python include="/home/buaacvis/anaconda3/envs/carla0.9.4/include/python3.5m/" -j ${CARLA_BUILD_CONCURRENCY} install 
